...
__device__ float* random_normal_vector(int length, curandState* state) {
    float* result = alloc_float(length);
    for (int vector_index = 0; vector_index < length; vector_index++) {
        result[vector_index] = curand_normal(state);
    }
    return result;
}

__device__ void add_noise(float* vec,
...
__device__ void update_importance_weights(float* weights,
                                          systemModel model,
                                          float* current_measurement,
                                          float* particles,
                                          int num_particles) {
...
__device__ void update_estimates(float* estimate,
                                 float* weights,
                                 float* particles,
                                 int num_particles,
...
__device__ float* resample_particles(float* particles,
                                     float* weights,
                                     int num_particles,
                                     int num_state_variables,
                                     curandState* states) {
...
__device__ void predict_particles_step(systemModel model,
                                       float* particles,
                                       int num_particles,
                                       curandState* states) {
    for (int particle_index = 0; particle_index < num_particles; particle_index++) {
        float* particle = &particles[particle_index * model.num_state_variables];
        float* process_estimate = model.step_process(particle);
        add_noise(process_estimate, model.process_noise_covariance_sqrt, model.num_state_variables, &states[particle_index]);
        memcpy(particle, process_estimate, model.num_state_variables * sizeof(float));
        free(process_estimate);
    }
}

__device__ float* initialize_particles(systemModel model,
                                       int num_particles,
                                       curandState* states) {
    ...
    for (int particle_index = 0; particle_index < num_particles; particle_index++) {
        ...
        add_noise(particle, model.initial_state_covariance_sqrt, model.num_state_variables, &states[particle_index]);
    }
    return particles;
}
